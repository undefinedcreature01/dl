...
> ! code and visualization [here !](notebooks/00_pytorch_activation_functions.ipynb)
...
| prediction/logit | output generated by a model when given input data  |
| loss/error/cost | difference between the output a model generates and the desired output |
| |   |

## Backpropagation

Backpropagation is a way to train an FNN based model. This is a gradient based - it adjusts the paramters of a model based on the error/loss between models predictions(its own output) and the actual desired output 
...
> <a href="notebooks/01_pytorch_loss_functions.ipynb" target="_blank">! code and visualization here !</a>
