...
Backpropagation is a way to train an FNN based model. This method adjusts the paramters of a model based on the error/loss between models predictions(its own output) and the actual desired output.
...
| $p$ | prediction |
| $p(y_i)$ =  $\hat{y}_i$ | models prediction(usually propability), where $y_i$ is the correct output |
| |   |
...
We calculate the mean (average) of the squared difference between each prediction and the desired ouput. The difference is squared to ensure both positive and negative values contribute equally to the overall loss.
...
| |   |
|---|---|
| positive class | the class being predicted - desired output/class  |
| negative class |  the class not being predicted  |
| |   |

$$ - \frac{1}{n} * Σ(y_i * log(\hat{y}_i) + (1 - y_i) * log(1 - \hat{y}_i)) $$

$y_i$ : truth ; binary value (either 1 or a 0) - the positive class
> respresents the "class" of the example

$\hat{y}_i$ : propability ; between 0 and 1 
> how sure is the model that the example belongs in the positive class
...
This is used to find the minimum of a loss function. It takes the loss
